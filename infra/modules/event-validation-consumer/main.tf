module "incoming_queue" {
    ...
    name = "${var.application}-${var.environment}-validation-incomming-queue"
    retry_count = 3
}
...
module "consumer_process_validation_event_lambda" {
  source               = "../constructs/lambda"
  lambda_handler       = "index.handler"
  lambda_function_name = "${var.application}-${var.environment}-processs-validation-events"
...
resource "aws_lambda_event_source_mapping" "incoming_event_source" {
  ...
  function_name    = module.consumer_process_validation_event_lambda.function_name
}
